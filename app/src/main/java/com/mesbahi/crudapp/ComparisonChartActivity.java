...
public class ComparisonChartActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<Float> consumptionValues = getConsumptionDataForComparison();
    ...
    private List<Float> getConsumptionDataForComparison() {
    ...
    private float getConsumptionForMonth(int userId, int month, int year) {
        ...
        Cursor cursor = dbHelper.getElectricityConsumptionForMonth(userId, month, year);

        if (cursor.moveToFirst()) {
            consumptionValue = cursor.getFloat(cursor.getColumnIndex(DBHelper.COLUMN_CONSUMPTION_VALUE));
        }
